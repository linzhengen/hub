...
**Document Evolution Policy:** This is a living document. It should be continuously updated to reflect the latest approved conventions and implementation policies. Any new standards or guidelines confirmed by the user should be promptly added to this guide to ensure it remains an accurate and evolving resource for development.

This document provides a guide for AI agents to understand and develop on the `hub` project.

## 1. Project Overview

This project is the API backend for `hub-web`.
...
- **`make build`**:
  - Executes `skaffold build` to build the container images. Information about the built images is output to `output.json`.

- **`make s-dev`**:
  - Executes `skaffold dev` to start development mode on Kubernetes (minikube). It watches for file changes and automatically rebuilds and redeploys.
...
- **`make tunnel`**:
  - Uses the `minikube service` command to create a tunnel from the local machine to the service (`hub-web`) running on minikube, making it accessible.
...
## Active Technologies
- Go 1.25.0+ (existing project standard) + go-zero (microservices), go.uber.org/dig (DI), golang-migrate (DB migrations), sqlc (SQL generation), buf (protobuf generation), testify (testing) (001-model-provider)
- MySQL with golang-migrate migrations, sqlc for type-safe queries (001-model-provider)

## Recent Changes
- 001-model-provider: Added Go 1.25.0+ (existing project standard) + go-zero (microservices), go.uber.org/dig (DI), golang-migrate (DB migrations), sqlc (SQL generation), buf (protobuf generation), testify (testing)
