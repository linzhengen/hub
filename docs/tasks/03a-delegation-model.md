...
- 委譲を作り、一覧し、失効できる。
- スコープの無い（`permission_ids` が空の）委譲を作れない。
  空を「全権」と読むと fail-open になるので、空は「何も許さない」である
  （ADR の補足を参照）。
- 失効が 1 秒以内に効く（`rbac_revisions` の結合テストで確認）。
- **自分以外を principal にした委譲を、誰も作れない。**「権限があれば作れる」ではない ——
  リクエストに principal のフィールドが無く、文脈の呼び出し元から取るので、
  他人の代理権を配ることは API が表現できない。
