...
// hub keeps its decisions in one place. The other candidates are kept only so
// that a repo that predates this convention is still found rather than gaining a
// second decisions directory beside the one it already has - which is the exact
// "two places for the same thing" this project avoids everywhere else.
const ADR_DIR = path.join('docs', 'decisions');

function detectAdrDir(repoRoot) {
  const candidates = [
    path.join(repoRoot, ADR_DIR),
...
function renderTemplate(raw, vars) {
  ...
  out = out.replace(/^(date:\s*)\{[^}]*\}[ \t]*$/m, `$1${vars.date}`);
  out = out.replace(
    /^(decision-makers:\s*)["']?\{[^}]*\}["']?[ \t]*$/m,
    `$1${vars.deciders || ''}`,
  );
  ...
  // Replace the H1 placeholder, whatever it says. It used to match the English
  // template's "{short title ...}" literally, which silently left the
  // placeholder in place once the templates were written in Japanese.
  out = out.replace(/^(#\s+)\{[^}]*\}[ \t]*$/m, `$1${vars.title}`);
...
function main() {
  ...
  else adrDir = detectAdrDir(repoRoot) || path.join(repoRoot, ADR_DIR);
