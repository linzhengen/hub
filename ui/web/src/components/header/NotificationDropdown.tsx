...
import { usePendingApprovals } from "@/hooks/useAccessRequests";

/**
 * The bell in the header.
 *
 * It shows one thing: access requests waiting on the signed-in user. An
 * approval keeps somebody else blocked, and the queue lives on a page nobody
 * has a reason to open, so this is what makes it noticed.
 *
 * The indicator appears only when something is actually waiting. A dot that is
 * always lit tells the reader nothing, and they stop looking at it - which
 * costs more than having no dot at all.
 */
  ...
  return (
    <div className="relative">
      <button
        aria-label={
          pending.length > 0
            ? `Notifications, ${pending.length} waiting on you`
            : "Notifications"
        }
            ...
            {pending.map((request) => (
              <li key={request.id}>
                ...
                >
                  ...
                  {/* The reason, not the ids: it is the part that says whether
                      this is worth opening now. What is actually being granted
                      is shown on the review screen, where it is agreed to. */}
